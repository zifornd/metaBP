...
//
include { PROKKA as PROKKA_BINS    } from '../modules/nf-core/modules/prokka/main'              addParams( options: modules['prokka']                )
include { PROKKA as PROKKA_ASSEMBLIES} from '../modules/nf-core/modules/prokka/main'            addParams( options: modules['prokka']                )
...
workflow MAG {
    ...
    */
    ...
    ch_short_reads_trimmomatic = TRIMMOMATIC.out.reads
    ...
    */
	...
	BOWTIE2_HOST_REMOVAL_ALIGN_TRIMMOMATIC (
        ...
        ch_software_versions = ch_software_versions.mix(BOWTIE2_HOST_REMOVAL_ALIGN_CUTADAPT.out.version.first().ifEmpty(null))
    }
    ...
    if(!params.keep_phix) {
        ...
        ch_short_reads = ch_short_reads_cutadapt.mix(ch_short_reads_trimmomatic)
    }
    ...
    ch_fastqc_trimmed = FASTQC_TRIMMED_CUTADAPT.out.zip.mix(FASTQC_TRIMMED_TRIMMOMATIC.out.zip)
    ...
    */
    ...
    //ch_short_reads_grouped = ch_short_reads_grouped_cutadapt.mix(ch_short_reads_grouped_trimmomatic)
    
    /*
    -------------------------------------------------------------------------------------------------
                           Assembly of reads from both QC tools simultaneously
    -------------------------------------------------------------------------------------------------
    */
    ...
    if (!params.skip_megahit){
        ...
        ch_assemblies_cutadapt = ch_megahit_assemblies_cutadapt.mix(ch_megahit_assemblies_cutadapt)
    ...
    */
    ...
    /*if (!params.single_end && !params.skip_spadeshybrid){
        ch_short_reads_spades_cutadapt
            .map { meta, reads -> [ meta.id, meta, reads ] }
            .set {ch_short_reads_spades_cutadapt_tmp}
	ch_short_reads_spades_trimmomatic
            .map { meta, reads -> [ meta.id, meta, reads ] }
            .set {ch_short_reads_spades_trimmomatic_tmp}
        ch_long_reads_spades
            .map { meta, reads -> [ meta.id, meta, reads ] }
            .combine(ch_short_reads_spades_cutadapt_tmp, by: 0)
            .map { id, meta_long, long_reads, meta_short, short_reads -> [ meta_short, long_reads, short_reads ] }
            .set { ch_reads_spadeshybrid }
	ch_long_reads_spades_trimmomatic
            .map { meta, reads -> [ meta.id, meta, reads ] }
            .combine(ch_short_reads_spades_trimmomatic_tmp, by: 0)
            .map { id, meta_long, long_reads, meta_short, short_reads -> [ meta_short, long_reads, short_reads ] }
            .set { ch_reads_spadeshybrid_trimmomatic }
	...
	ch_assemblies = ch_assemblies_cutadapt.mix(ch_assemblies_trimmomatic,ch_megahit_assemblies_trimmomatic,ch_megahit_assemblies_cutadapt)
	...
	SPADESHYBRID_TRIMMOMATIC ( ch_reads_spadeshybrid_trimmomatic )
        SPADESHYBRID_TRIMMOMATIC.out.assembly
            .map { meta, assembly ->
                def meta_new = meta.clone()
                meta_new.assembler  = "SPAdesHybrid"
                [ meta_new, assembly ]
            }
            .set { ch_spadeshybrid_assemblies_trimmomatic }
        ch_assemblies_trimmomatic = ch_assemblies_trimmomatic.mix(ch_spadeshybrid_assemblies_trimmomatic)
        ch_software_versions = ch_software_versions.mix(SPADESHYBRID_TRIMMOMATIC.out.version.first().ifEmpty(null))
    }*/

    /*
    -------------------------------------------------------------------------------------------------
                           Assembly QC of the 4 combinations of outputs
    -------------------------------------------------------------------------------------------------
    */
	...
	ch_quast_multiqc = QUAST_CUTADAPT_SPADES.out.qc.mix (
				QUAST_TRIMMOMATIC_SPADES.out.qc,
				QUAST_CUTADAPT_MEGAHIT.out.qc,
				QUAST_TRIMMOMATIC_MEGAHIT.out.qc )
    ...
    */
    ...
    if (!params.skip_prodigal){
        PRODIGAL (
            ch_megahit_assemblies_cutadapt.mix (
    ...
    */
	...
	ch_bowtie2_assembly_multiqc = METABAT2_BINNING_CUTADAPT_MEGAHIT.out.bowtie2_assembly_multiqc.mix (
					METABAT2_BINNING_TRIMMOMATIC_MEGAHIT.out.bowtie2_assembly_multiqc,
					METABAT2_BINNING_CUTADAPT_SPADES.out.bowtie2_assembly_multiqc,
					METABAT2_BINNING_TRIMMOMATIC_SPADES.out.bowtie2_assembly_multiqc )
	ch_software_versions = ch_software_versions.mix(METABAT2_BINNING_CUTADAPT_SPADES.out.bowtie2_version.first().ifEmpty(null))
	    ...
	    ch_busco_summary_trimmomatic_megahit = BUSCO_QC_TRIMMOMATIC_MEGAHIT.out.summary
            ...
            ch_busco_multiqc = BUSCO_QC_CUTADAPT_SPADES.out.multiqc.mix (
				BUSCO_QC_CUTADAPT_MEGAHIT.out.multiqc,
				BUSCO_QC_TRIMMOMATIC_SPADES.out.multiqc,
				BUSCO_QC_TRIMMOMATIC_MEGAHIT.out.multiqc )
	...
	//ch_gtdbtk_summary_trimmomatic_spades = Channel.empty()
	    ...
	    GTDBTK (
                METABAT2_BINNING_TRIMMOMATIC_SPADES.out.bins.mix ( 
			METABAT2_BINNING_TRIMMOMATIC_MEGAHIT.out.bins,
			METABAT2_BINNING_CUTADAPT_SPADES.out.bins,
			METABAT2_BINNING_CUTADAPT_MEGAHIT.out.bins ),
                ch_busco_summary_trimmomatic_spades.mix (
			ch_busco_summary_trimmomatic_megahit,
			ch_busco_summary_cutadapt_spades,
			ch_busco_summary_cutadapt_megahit ),
                ch_gtdb
	    ...
	    GTDBTK_SUMMARY_SPLIT (
           ...
           ch_gtdbtk_summary_cutadapt_spades = GTDBTK_SUMMARY_SPLIT.out.cut_spa_gtdbsum
           ch_gtdbtk_summary_cutadapt_megahit = GTDBTK_SUMMARY_SPLIT.out.cut_mg_gtdbsum
           ch_gtdbtk_summary_trimmomatic_spades = GTDBTK_SUMMARY_SPLIT.out.trim_spa_gtdbsum
           ch_gtdbtk_summary_trimmomatic_megahit = GTDBTK_SUMMARY_SPLIT.out.trim_mg_gtdbsum
        }
	    ...
	    ch_bin_depths_summary = METABAT2_BINNING_TRIMMOMATIC_MEGAHIT.out.depths_summary.mix (
					METABAT2_BINNING_TRIMMOMATIC_SPADES.out.depths_summary,
					METABAT2_BINNING_CUTADAPT_MEGAHIT.out.depths_summary,
					METABAT2_BINNING_CUTADAPT_SPADES.out.depths_summary ) 
	    ch_busco_summary = ch_busco_summary_trimmomatic_megahit.mix (
					ch_busco_summary_trimmomatic_spades,
					ch_busco_summary_cutadapt_megahit,
					ch_busco_summary_cutadapt_spades )
	    ch_quast_bins_summary = ch_quast_bins_summary_trimmomatic_megahit.mix (
					ch_quast_bins_summary_trimmomatic_spades,
					ch_quast_bins_summary_cutadapt_megahit,
					ch_quast_bins_summary_cutadapt_spades )
         ...
         */
             METABAT2_BINNING_CUTADAPT_MEGAHIT.out.bins.mix (
	     METABAT2_BINNING_CUTADAPT_SPADES.out.bins,
	     METABAT2_BINNING_TRIMMOMATIC_MEGAHIT.out.bins,
	     METABAT2_BINNING_TRIMMOMATIC_SPADES.out.bins )
	    .transpose()
		...
		ch_spades_assemblies_trimmomatic ),
            ...
            ch_software_versions = ch_software_versions.mix(PROKKA_BINS.out.versions.first().ifEmpty(null))
        }
    }
    ...
    MULTIQC (
        ...
        FASTQC_TRIMMED_CUTADAPT.out.zip.mix(FASTQC_TRIMMED_TRIMMOMATIC.out.zip).collect{it[1]}.ifEmpty([]),
