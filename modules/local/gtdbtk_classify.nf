...
process GTDBTK_CLASSIFY {
    ...
    if (workflow.containerEngine == 'singularity' && !params.singularity_pull_docker_container) {
        ...
        container "quay.io/biocontainers/gtdbtk:2.1.0--pyhdfd78af_5"//2.0.0--pyhdfd78af_0"
    }
    ...
    path "classify/gtdbtk.${meta.assembler}-${meta.id}.*.classify.tree.gz"   , emit: tree
    path "identify/gtdbtk.${meta.assembler}-${meta.id}.*.markers_summary.tsv", emit: markers
    path "align/gtdbtk.${meta.assembler}-${meta.id}.*.msa.fasta.gz"       , emit: msa
    path "align/gtdbtk.${meta.assembler}-${meta.id}.*.user_msa.fasta.gz"     , emit: user_msa
    path "align/gtdbtk.${meta.assembler}-${meta.id}.*.filtered.tsv"       , emit: filtered
    path "gtdbtk.${meta.assembler}-${meta.id}.log"                  , emit: log
    path "gtdbtk.${meta.assembler}-${meta.id}.warnings.log"         , emit: warnings
    path "identify/gtdbtk.${meta.assembler}-${meta.id}.failed_genomes.tsv"   , emit: failed
    ...
    gzip "classify/gtdbtk.${meta.assembler}-${meta.id}".*.classify.tree #> "gtdbtk.${meta.assembler}-${meta.id}".*.classify.tree.gz 
    #gzip "align/gtdbtk.${meta.assembler}-${meta.id}".*.msa.fasta > "gtdbtk.${meta.assembler}-${meta.id}".*.msa.fasta.gz
